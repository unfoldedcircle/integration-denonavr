...
class TestSimpleCommandMappings(unittest.TestCase):
    ...
    def test_get_simple_commands_denon(self):
        ...
        for cmd in commands:
            ...
            self.assertIn(dtype, {simplecommand.DeviceType.ALL, simplecommand.DeviceType.DENON})
            self.assertEqual(protocol, simplecommand.DeviceProtocol.ALL)
        ...
        self.assertTrue(denon_specific_found, "No Denon-specific command found")
        self.assertFalse(marantz_specific_found, "Marantz-specific command found for Denon device")
    ...
    def test_get_simple_commands_marantz(self):
        ...
        for cmd in commands:
            protocol, dtype = self.get_command_info(cmd)
            self.assertIn(dtype, {simplecommand.DeviceType.ALL, simplecommand.DeviceType.MARANTZ})
            self.assertEqual(protocol, simplecommand.DeviceProtocol.ALL)
            if dtype == simplecommand.DeviceType.MARANTZ:
                marantz_specific_found = True
            if dtype == simplecommand.DeviceType.DENON:
        ...
        self.assertTrue(marantz_specific_found, "No Marantz-specific command found")
        self.assertFalse(denon_specific_found, "Denon-specific command found for Marantz device")
